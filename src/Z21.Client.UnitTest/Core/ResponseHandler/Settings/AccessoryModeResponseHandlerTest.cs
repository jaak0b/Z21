...
{
  public class AccessoryModeResponseHandlerTest
  {
    private AccessoryModeResponseHandler _handler;
    ...
    {
      byte[] validResponse = [0x07, 0x00, 0x70, 0x00, 0x00, 0x0C, 0x00];
    ...
    [Test]
    [TestCase(new byte[] { 0x07, 0x00, 0x71, 0x00, 0x00, 0x0C, 0x00 }, TestName = "Not Responsible")]
    ...
    {
      byte[] response = [0x07, 0x00, 0x70, 0x00, 0x00, 0x0C, modeByte];

      DecoderModeReceivedEventArgs? receivedArgs = null;
      AccessoryModeResponseHandler? handler = null;
      _handler.OnAccessoryModeReceived += (sender, args) =>
                                     {
                                       receivedArgs = args;
                                       handler = sender as AccessoryModeResponseHandler;
                                     };
