...
{
  public class CvNackResponseHandlerTest
  {
    private CvNackResponseHandler _handler = null!;
    ...
    {
      byte[] validResponse = [0x07, 0x00, 0x40, 0x00, 0x61, 0x13, 0x72];
      Assert.That(_handler.CanHandle(validResponse), Is.True);
    }

    [Test]
    [TestCase(new byte[] { 0x07, 0x00, 0x40, 0x00, 0x61, 0x12, 0x73 }, TestName = "Short circuit nack")]
    ...
    {
      byte[] response = [0x07, 0x00, 0x40, 0x00, 0x61, 0x13, 0x72];

      bool raised = false;
      _handler.OnCvNackReceived += (_, _) => raised = true;
